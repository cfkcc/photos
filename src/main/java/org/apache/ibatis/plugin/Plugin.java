...
import com.photo.plugin.PagePlugin;
import com.photo.plugin.PageView;
import com.photo.util.Common;
import com.photo.util.EhcacheUtils;
	...
	public static final String joinSql(Connection connection, MappedStatement mappedStatement, BoundSql boundSql,
		...
		if (Configuration.FINDBYWHERE.equals(sqlId)) {
			...
			for (Entry<String, Object> entry : formMap.entrySet()) {
				if (!Configuration.LY_TABLE.equals(entry.getKey()) && null != entry.getValue() && !"_t".equals(entry.getKey()))
